...
#### Dynamic System-Wide Font Switching (`[ T ]` Navbar Toggle)
The system supports interactive font switching via the navbar `[ T ]` icon button. Font mode persists in `localStorage` and cascades across all components:
- `DEFAULT`: Nothing Space Stack (`Space Grotesk`, `Space Mono`, `Doto`)
- `INTER`: Inter (Modern Web App font)
- `JAKARTA`: Plus Jakarta Sans (Clean Geometric Sans font)

#### Minimum Font Size Mandate (10px Minimum)
- **Smallest font size across the entire application MUST be 10px (`0.625rem`).**
- Font sizes smaller than 10px (e.g. 6px, 7px, 8px, 9px) are strictly prohibited across all screens, readouts, labels, and badges.
...
**Monetary & Price Display Standard**: All transaction prices, monetary figures, expenditure totals, receipt amounts, and currency readouts on the Dashboard MUST be styled in **vibrant orange (`var(--orange)` / `#FF5C00`)** font color.
...
### 2.10 Responsiveness Standard (Mobile, Tablet, Desktop)

Ensure every screen renders seamlessly across all viewport dimensions:

| Viewport | Breakpoint | Sidebar & Header Behavior | Content Grid & Typography |
|----------|------------|---------------------------|---------------------------|
| **Desktop** | ≥ 992px | Expanded sticky left sidebar (`250px` width) with full text labels & system sublinks. | Multi-column asymmetric split (`1.2fr 1fr` or 3-column grid). Full `--display-xl` scale. |
| **Tablet** | 768px – 992px | Sticky left sidebar auto-collapses to compact icon mode (`70px` width) with vertical control stack. | Main content stacks vertically into a single column (`flex-direction: column`). Tablet type scale. |
| **Mobile** | ≤ 768px | Sticky left sidebar stays compact (`70px` width). Top header wraps control buttons cleanly. | 100% fluid card width (`max-width: 100%`). `word-break: break-word` on headings. Table `overflow-x: auto`. |

#### Essential Responsiveness Rules:
1. **Zero Text Clipping / Overflow**: All display headings must use `word-break: break-word` and `overflow-wrap: anywhere`.
2. **No Horizontal Scroll on Page Body**: Tables, cards, and code readouts must use inner `overflow-x: auto` wrappers so page-level scroll remains strictly 100% vertical.
3. **Navbar & Footer Standardization**: Top header nav contains (`HOME`, `DASHBOARD`, `ONBOARDING`, `LOGIN`, `REGISTER`). `TECH STACK` and `PRICING` buttons sit in the footer above the copyright notice.
...
- No native browser OS `<select>` popup dialogs. ALWAYS use the custom expanding dropdown component (`CustomDropdown.tsx`) with `[●]` trigger indicators, `[✓]` selection markers, and custom inline list expansion.
